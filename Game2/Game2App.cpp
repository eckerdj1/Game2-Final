...
void Game2App::initApp()
{
	...
	splash.init(md3dDevice, 100.0f, White);
	...
	for (int i=numberOfSpotLights; i<30; ++i)
	{
...
void Game2App::updateScene(float dt)
{
	...
	if (gameState == TITLE) {
		...
		/*splash.setPosition(Vector3(0.0f, 100.0f, 0.0f));
		splash.update(dt);
		...
		camPos = Vector3(0.0f, 100.0f, -200.0f);
		camPos -= CameraDirection * 300 / camZoom;

		target = Vector3(0.0f, 100.0f, 0.0f);
		if (gameTime > 15.0f)*/
			gameState = HOWTO;
	}
